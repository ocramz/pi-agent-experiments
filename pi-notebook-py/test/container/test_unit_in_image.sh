...
#
# Most of these cases spawn a real interpreter, and each builds its Kernel in a
...
# and HOME has been forced there. On a dev machine every one of those steps is
# pre-warmed; here there is nothing, which is the point.
#
...
if [ "${passed:-0}" -ge 20 ] 2>/dev/null; then
	ok "the suite actually ran ($passed tests)"
else
	fail "the suite actually ran" "expected at least 20 passing tests" \
